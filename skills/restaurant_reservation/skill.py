    ...
    def _create_reservation_handler(self, args, raw_data):
        ...
        try:
            ...
            with app.app_context():
                ...
                # Add explicit SMS action for SWAIG
                try:
                    ...
                    # Build SMS body for SWAIG action
                    sms_body = f"🍽️ Bobby's Table Reservation Confirmed!\n\n"
                    sms_body += f"Name: {args['name']}\n"
                    sms_body += f"Date: {args['date']}\n"
                    sms_body += f"Time: {time_12hr}\n"
                    party_text = "person" if args['party_size'] == 1 else "people"
                    sms_body += f"Party Size: {args['party_size']} {party_text}\n"
                    sms_body += f"Reservation Number: {reservation.reservation_number}\n"

                    if args.get('special_requests'):
                        sms_body += f"Special Requests: {args['special_requests']}\n"

                    if total_reservation_amount > 0:
                        sms_body += f"\nPre-Order Total: ${total_reservation_amount:.2f}\n"
                        sms_body += f"Your food will be prepared when you arrive!\n"
                    ...
                    # Add SMS action to SWAIG result
                    result.send_sms(
                        to_number=args['phone_number'],
                        from_number=signalwire_from_number,
                        body=sms_body
                    )
                    print(f"📱 Added SWAIG SMS action for reservation {reservation.reservation_number}")

                except Exception as sms_action_error:
                    print(f"📱 Warning: Could not add SWAIG SMS action: {sms_action_error}")
                    # Don't fail the reservation if SMS action fails
